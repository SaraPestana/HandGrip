...
def plotClusters(y_pred, OSignal, time, XPCA, n_clusters):
	...
	for s in range(0, n_clusters):
		...
		sns.distplot(XPCA[np.where(y_pred == s)[0], 0], bins=50, color=color, kde=True, ax=g.ax_marg_x)
		sns.distplot(XPCA[np.where(y_pred == s)[0], 1], bins=50, color=color, kde=True, vertical=True,
		             ax=g.ax_marg_y)
...
def plotDistanceMetric(time, signal, signalNoise, Prediction, Spec, Sens, CorrectPeriod, sizes):
